...
Once the started, Enforcers will briefly disconnect and their status will transition from "disconnected migration running" back to "connected"
...
`apoctl enforcer switch-to --target-version <version> --confirm` at the target host.
